...
def init_db():
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS free_user_limits
                     (user_id INTEGER PRIMARY KEY, files_uploaded INTEGER DEFAULT 0, 
...
def check_free_user_limit(user_id):
    ...
    if result:
        files_uploaded, first_upload_date = result
        if first_upload_date:
            ...
            if (datetime.now() - first_date).days > FREE_USER_DAYS:
                ...
                conn.close()
                return True, None
            elif files_uploaded >= FREE_USER_LIMIT:
                days_left = FREE_USER_DAYS - (datetime.now() - first_date).days
                conn.close()
                return False, f"Limit reached! You've uploaded {FREE_USER_LIMIT} files. Wait {days_left} days or contact @{YOUR_USERNAME.replace('@', '')} to renew."
...
def update_free_user_upload(user_id):
    ...
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('SELECT files_uploaded FROM free_user_limits WHERE user_id = ?', (user_id,))
    result = c.fetchone()
    
    if result:
        c.execute('UPDATE free_user_limits SET files_uploaded = files_uploaded + 1 WHERE user_id = ?', (user_id,))
    else:
        c.execute('INSERT INTO free_user_limits (user_id, files_uploaded, first_upload_date) VALUES (?, 1, ?)',
...
def ban_user_db(user_id, reason=""):
    with DB_LOCK:
        ...
        c = conn.cursor()
        try:
            c.execute('INSERT OR REPLACE INTO banned_users (user_id, ban_date, reason) VALUES (?, ?, ?)',
...
def add_active_user(user_id):
    ...
    with DB_LOCK:
        ...
        c = conn.cursor()
        try:
            c.execute('INSERT OR IGNORE INTO active_users (user_id, join_date) VALUES (?, ?)',
                      (user_id, datetime.now().isoformat()))
            conn.commit()
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        admin_buttons = [
            ...
            types.InlineKeyboardButton('🚫 Ban/Unban User', callback_data='ban_unban_menu'),
            types.InlineKeyboardButton('👁 View User Files', callback_data='view_user_files_menu')
        ]
...
def create_ban_unban_menu():
    ...
    markup.row(types.InlineKeyboardButton('📋 Banned Users List', callback_data='list_banned_users'))
    markup.row(types.InlineKeyboardButton('🔙 Back to Main', callback_data='back_to_main'))
    return markup
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        update_free_user_upload(user_id)
...
def handle_js_file(file_path, script_owner_id, user_folder, file_name, message):
    try:
        ...
        update_free_user_upload(script_owner_id)
...
def _logic_send_welcome(message):
    ...
    if is_user_banned(user_id):
        bot.send_message(chat_id, "🚫 You are banned from using this bot. Contact admin for more info.")
        return
...
def _logic_upload_file(message):
    ...
    if is_user_banned(user_id):
        bot.reply_to(message, "🚫 You are banned. Cannot upload files.")
        return
    ...
    can_upload, error_msg = check_free_user_limit(user_id)
    if not can_upload:
        bot.reply_to(message, f"⚠️ {error_msg}\n\nContact: @{YOUR_USERNAME.replace('@', '')} to upgrade.")
        return
...
def _logic_statistics(message):
    ...
    for script_key_iter, script_info_iter in list(bot_scripts.items()):
        s_owner_id_str = script_key_iter.split('_')[0]
        try:
...
def _logic_ban_unban_menu(message):
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "Admin permissions required.")
        return
    bot.reply_to(message, "Ban/Unban User Management", reply_markup=create_ban_unban_menu())

def _logic_view_user_files(message):
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "Admin permissions required.")
        return
    msg = bot.reply_to(message, "Enter User ID to view their files.\n/cancel to abort.")
...
def process_view_user_files(message):
    ...
    if message.text and message.text.lower() == '/cancel':
        bot.reply_to(message, "View files cancelled.")
        return
    
    try:
        target_user_id = int(message.text.strip())
        files_info = get_user_files_info(target_user_id)
        
        if not files_info:
            bot.reply_to(message, f"User `{target_user_id}` has no uploaded files.")
            return
        
        response = f"Files for User `{target_user_id}`:\n\n"
        for idx, file_info in enumerate(files_info, 1):
            status = "🟢 Running" if file_info['is_running'] else "🔴 Stopped"
            response += f"{idx}. `{file_info['file_name']}` ({file_info['file_type']}) - {status} | Size: {file_info['file_size']}\n"
        ...
        bot.reply_to(message, "Invalid User ID. Please send a valid numerical ID.")
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "🚫 Ban/Unban": _logic_ban_unban_menu,
    "👁 View User Files": _logic_view_user_files
}
...
def command_ban(message):
    ...
    try:
        ...
        try:
            bot.send_message(target_id, f"🚫 You have been banned from the bot.\nReason: {reason}\nContact: @{YOUR_USERNAME.replace('@', '')}")
        except:
            pass
    except ValueError:
        bot.reply_to(message, "Invalid user ID. Use: /ban <user_id> [reason]")
...
def command_unban(message):
    ...
    try:
        ...
        if unban_user_db(target_id):
            ...
            try:
                bot.send_message(target_id, "✅ You have been unbanned from the bot. Welcome back!")
            except:
                pass
        else:
            bot.reply_to(message, f"User `{target_id}` is not banned or not found.")
    except ValueError:
        bot.reply_to(message, "Invalid user ID. Use: /unban <user_id>")

@bot.message_handler(commands=['bannedlist'])
def command_banned_list(message):
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "Admin permissions required.")
        return
    if not banned_users:
    ...
    banned_list_str = "Banned Users:\n\n"
    for uid in banned_users:
        banned_list_str += f"- `{uid}`\n"
    
    if len(banned_list_str) > 4000:
        for x in range(0, len(banned_list_str), 4000):
            bot.reply_to(message, banned_list_str[x:x+4000], parse_mode='Markdown')
    else:
        bot.reply_to(message, banned_list_str, parse_mode='Markdown')
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data == 'ban_unban_menu':
            admin_required_callback(call, ban_unban_menu_callback)
        elif data == 'ban_user':
            admin_required_callback(call, ban_user_init_callback)
        elif data == 'unban_user':
            admin_required_callback(call, unban_user_init_callback)
        elif data == 'list_banned_users':
            admin_required_callback(call, list_banned_users_callback)
        elif data == 'view_user_files_menu':
            admin_required_callback(call, view_user_files_menu_callback)
        elif data == 'mpx_ai':
...
def upload_callback(call):
    ...
    can_upload, error_msg = check_free_user_limit(user_id)
    if not can_upload:
        bot.answer_callback_query(call.id, f"⚠️ {error_msg}", show_alert=True)
        return
...
def speed_callback(call):
    ...
    try:
        ...
        try:
            bot.edit_message_text("Main Menu", chat_id, call.message.message_id, reply_markup=create_main_menu_inline(user_id))
        except Exception:
            pass
...
def process_add_admin_id(message):
    ...
    try:
        ...
        if new_admin_id in admin_ids:
            bot.reply_to(message, f"User `{new_admin_id}` already Admin.")
            return
...
def process_remove_admin_id(message):
    ...
    try:
        ...
        if admin_id_remove == OWNER_ID:
            bot.reply_to(message, "Owner cannot remove self.")
            return
        if admin_id_remove not in admin_ids:
            bot.reply_to(message, f"User `{admin_id_remove}` not Admin.")
            return
...
def process_remove_subscription_id(message):
    ...
    try:
        ...
        if sub_user_id_remove not in user_subscriptions:
            bot.reply_to(message, f"User `{sub_user_id_remove}` no active sub in memory.")
            return
    ...
    try:
        ...
        if sub_user_id_check in user_subscriptions:
            ...
            if expiry_dt:
                ...
                bot.reply_to(message, f"User `{sub_user_id_check}` in sub list, but expiry missing. Re-add if needed.")
        else:
            bot.reply_to(message, f"User `{sub_user_id_check}` no active sub record.")
    except ValueError:
        bot.reply_to(message, "Invalid ID. Send numerical ID or /cancel.")
...
def ban_unban_menu_callback(call):
    bot.answer_callback_query(call.id)
    try:
        bot.edit_message_text("Ban/Unban User Management\nSelect action:",
                              call.message.chat.id, call.message.message_id, reply_markup=create_ban_unban_menu())
    except Exception as e:
...
def process_ban_user(message):
    ...
    try:
        ...
        bot.reply_to(message, "Invalid format. Use: `user_id reason` or /cancel.")
        msg = bot.send_message(message.chat.id, "Enter User ID and reason, or /cancel.")
        bot.register_next_step_handler(msg, process_ban_user)
...
def process_unban_user(message):
    ...
    try:
        ...
        bot.reply_to(message, "Invalid ID. Send numerical ID or /cancel.")
        msg = bot.send_message(message.chat.id, "Enter User ID to unban, or /cancel.")
        bot.register_next_step_handler(msg, process_unban_user)
...
def list_banned_users_callback(call):
    ...
    if not banned_users:
        try:
            bot.edit_message_text("No banned users.", call.message.chat.id, call.message.message_id, reply_markup=create_ban_unban_menu())
        except:
            pass
        return
    ...
    try:
        bot.edit_message_text(banned_list_str, call.message.chat.id, call.message.message_id, reply_markup=create_ban_unban_menu(), parse_mode='Markdown')
    except Exception as e:
        logger.error(f"Error listing banned users: {e}")

def view_user_files_menu_callback(call):
    bot.answer_callback_query(call.id)
